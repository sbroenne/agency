...
- 2026-03-17: Visual acceptance harness prepared at `tooling/tests/visual-acceptance.test.mjs`. Run with `npm run test:visual` (builds the site first). The test starts its own preview server on port 4322, captures screenshots to `tooling/tests/screenshots/visual-acceptance/`, and asserts the six acceptance criteria: lighter shell, rose CTA pair, no backdrop-blur, no cyan accent, tight border-radius (≤20 px), and no explanatory style sentence. The `test` script was narrowed to `tooling/tests/registry.test.mjs` so the visual harness (which needs a browser) does not run in CI automatically.
